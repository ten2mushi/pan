...
test "RuntimeResampler: drift-free streaming, resamples a sine vs an analytic oracle" {
    const gpa = std.testing.allocator;
    const pi = std.math.pi;
    inline for (.{ .{ 2, 1 }, .{ 1, 2 }, .{ 160, 147 }, .{ 147, 160 } }) |pq| {
        const p = pq[0];
        const q = pq[1];
        var rs = try RuntimeResampler.init(gpa, p, q, 1, 16);
        defer rs.deinit(gpa);
        const f0: f64 = 0.01; // cycles per INPUT sample, deep in the passband
        const want = 128;
        const up = @max(p, q);
        // The FRACTIONAL group delay (output samples); latency() is its integer
        // floor for PDC, but the analytic phase needs the exact fractional value.
        const gd: f64 = @as(f64, @floatFromInt(rs.half * up + p)) / @as(f64, q);
        var in_pos: usize = 0; // continuous input index
        var out_global: usize = 0;
        for (0..8) |_| {
            const need = rs.needed_input(want); // EXACT — drift-free
            const in = try gpa.alloc(f32, need);
            defer gpa.free(in);
            for (in, 0..) |*x, i| x.* = @floatCast(@sin(2.0 * pi * f0 * @as(f64, @floatFromInt(in_pos + i))));
            in_pos += need;
            var out: [want]f32 = undefined;
            rs.process(in, need, &out, want);
            for (out, 0..) |y, j| {
                const J = out_global + j;
                if (@as(f64, @floatFromInt(J)) < 2.0 * gd) continue; // skip the priming transient
                // output J reconstructs the tone at the output rate, delayed by gd.
                const want_v: f64 = @sin(2.0 * pi * f0 * (@as(f64, @floatFromInt(J)) - gd) * @as(f64, q) / @as(f64, p));
                try testing.expectApproxEqAbs(want_v, @as(f64, y), 5e-2);
            }
            out_global += want;
        }
        // Over 8·128 outputs the total inputs consumed equals the analytic count
        // (no drift): in_pos == floor((8·128)·q/p) within the phase remainder.
        const total_out: f64 = 8.0 * 128.0;
        const expected_in = @as(usize, @intFromFloat(total_out * @as(f64, q) / @as(f64, p)));
        try testing.expect(in_pos == expected_in or in_pos == expected_in + 1);
    }
}
